...
int draw( )
{
  ...
  Vector t(0.65, 0, 0) ;
  t = RotationZ(-0.05 * elapsed)(t) ;

  b1.T = Translation(t) ;
  b2.T = Translation(-t) ;

  /*Decommenter dans un second temps*/
  b1.T = b1.T * RotationZ(-0.03 * elapsed) * RotationY(0.05 * elapsed);
  b2.T = b2.T * RotationZ(-0.02 * elapsed) * RotationY(0.05 * elapsed);
